...
3. Extract the usable configuration file from the server and use it to apply
   the schema migrations and begin the initial import step:

   ```bash
   $ nix shell nixpkgs#gitlab-container-registry -c \
       registry database migrate up config.toml
   $ nix shell nixpkgs#gitlab-container-registry -c \
       registry import --step-one config.toml
   ```
